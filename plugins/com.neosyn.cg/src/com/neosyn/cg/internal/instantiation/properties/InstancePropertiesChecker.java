...
public class InstancePropertiesChecker extends PropertiesChecker {
	...
	private void updateClocksInstantiated(Instance instance) {
		...
		// Skip clock update if entity is not resolved (e.g., external entity in CLI mode)
		if (entity == null) {
			return;
		}
	...
	private void updateResets(Instance instance) {
		...
		Entity entity = instance.getEntity();
		// Skip reset update if entity is not resolved (e.g., external entity in CLI mode)
		if (entity == null) {
			return;
		}
		JsonArray entityResets = entity.getProperties().getAsJsonArray(PROP_RESETS);
