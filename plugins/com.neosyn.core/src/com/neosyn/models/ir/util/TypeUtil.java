	...
	public static boolean canAssign(Type src, Type dst) {
		...
		if (src.isArray() && dst.isArray()) {
			...
			if (canAssign(typeSrc.getElementType(), typeDst.getElementType())) {
				...
				// Require the same dimensions. (The previous loop never advanced
				// its iterators, so it spun forever the moment an array type
				// reached it — e.g. an array-literal local initializer under the
				// TypeChecker, which only began running on the CLI path once
				// validation was wired in.)
				while (itD.hasNext() && itS.hasNext()) {
					if (!itD.next().equals(itS.next())) {
						// same rank so far but a differing dimension
						return false;
					}
				}
				// assignable only when both ranks are fully consumed
				return !itD.hasNext() && !itS.hasNext();
			}
		}
